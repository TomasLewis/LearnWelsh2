...
$("#backButton").on("touchend mouseup",function(e){e.preventDefault();goBack()});
landed();
...
$("#selButtonFull").on("touchstart mouseenter",function(e){e.preventDefault();setStatusString(modStrings[8])}).on("mouseleave",function(){setDefaultString()}).on("touchend mouseup",function(e){e.preventDefault();clickS.play();second()});

$("#goGames").on("touchstart mouseenter",function(e){e.preventDefault();setStatusString(modStrings[10])}).on("mouseleave",function(){setDefaultString()}).on("touchend mouseup",function(e){e.preventDefault();clickS.play();loadPage("games");});

$("#introButton").on("touchstart mouseenter",function(e){e.preventDefault();setStatusString(modStrings[68])}).on("mouseleave",function(){setDefaultString()}).on("touchend mouseup",function(e){e.preventDefault();
clickS.play();$("#pageInfoFX").css("display","block");});

$("#topicsSign").on("touchstart mouseenter",function(e){e.preventDefault();setStatusString(modStrings[11])}).on("mouseleave",function(){setDefaultString()}).on("touchend mouseup",function(e){e.preventDefault();clickS.play();loadPage("topics");});
}

function sharedEndInterface(i){

$("#introBlueGoArra").off("touchstart mouseenter mouseleave touchend mouseup");

myString='<div id="introBlueTitle"></div><div id="introBlueText"></div><div id="introBlueAgain"><div class="arraSign">'+arra206+'<div class="arraLabel206">'+modStrings[57]+"</div></div></div>",myString+="whatIsIt"!==i?'<div id="introBlueGoArra">'+modStrings[59]+"</div>":'<div id="introBlueGoArra">'+modStrings[69]+"</div>",$("#contentSign").html(myString);

$("#introBlueGoArra").on("touchstart mouseenter",function(e){e.preventDefault();myString=$(this).text(),setStatusString(myString)}).on("mouseleave",function(){setDefaultString()}).on("touchend mouseup",function(e){e.preventDefault();"yesNo"==i?fourth():"eitherOr"==i?fifth():"introLang"==i?third():(myString=logamesURL,window.location.href=myString)});

$("#introBlueAgain").on("touchstart mouseenter",function(e){e.preventDefault();chngArraSign($(this),"#fff","#000");myString=$(this).text(),setStatusString(myString)}).on("mouseleave",function(){chngArraSign($(this),groundCol,strokeCol);setDefaultString()}).on("touchend mouseup",function(e){e.preventDefault();"yesNo"==i?third():"eitherOr"==i?fourth():"introLang"==i?second():fifth()});
}

function sharedInterface(i){

$("#introBlueGoArra").off("touchstart mouseenter mouseleave touchend mouseup");
$("#a1,#a2,#a3,#a4").off("touchstart mouseenter mouseleave touchend mouseup");
...
$(".arraSign").on("touchstart mouseenter",function(e){e.preventDefault();chngArraSign($(this),"#fff","#000")}).on("touchend mouseleave",function(e){e.preventDefault();chngArraSign($(this),groundCol,strokeCol)});

$("#introBlueGoArra").on("touchstart mouseenter",function(e){e.preventDefault();myString=$(this).text(),setStatusString(myString)}).on("touchend mouseleave",function(e){e.preventDefault();setDefaultString()});

$("#a1,#a2,#a3,#a4").on("touchstart mouseenter",function(e){e.preventDefault();myString=$(this).text(),setStatusString(myString)}).on("mouseleave",function(){setDefaultString()}).on("touchend mouseup",function(e){e.preventDefault();clickS.play();var i=$(this).attr("id");"a1"==i?second():"a2"==i?third():"a3"==i?fourth():fifth()});

myString=1==i?modStrings[13]:2==i?modStrings[14]:3==i?modStrings[15]:modStrings[16],$("#a"+i).html('<div class="navArraSelected"><p style="margin-left:22px;line-height:30px">'+myString+"</p></div>").off("touchstart mouseenter mouseleave touchend mouseup");
...
$("#transSign").on("touchstart mousedown",function(i){return i.stopPropagation(),i.preventDefault(),i.handled===!0?!1:($("#introLabel").text(translate(picNum,1)),reduceToHtWdth("#introLabel"),i.handled=!0,void 0)}).on("touchend mouseup",function(i){return i.stopPropagation(),i.preventDefault(),i.handled===!0?!1:($("#introLabel").text(translate(picNum,0)),reduceToHtWdth("#introLabel"),i.handled=!0,void 0)});

$("#introNext,#introPrev").on("touchstart mouseenter",function(e){e.preventDefault();
console.log('introNext or prev, t-start or m-enter');
$(this).css("background-color","#000");
}).on("touchend mouseleave mouseup",function(e){e.preventDefault();
console.log('introNext or prev, t-end or m-up-leave');
$(this).css("background-color",groundCol);
})
$("#introNext").on("click",function(){picNum<numPics-1?(picNum+=1,i(picNum)):t()});
$("#introPrev").on("click",function(){picNum>0&&(picNum-=1,i(picNum))});
...
console.log('prepActivityarrays, picNum='+picNum);
}
...
function havingAnsweredWII(){for(setTimeout(function(){havingAnsweredWIITwo()},400),1==correct?(highPop.play(),starRight(2),rightAnswers+=1):(wrongOverlay("whatIsIt"),wrong.play(),starWrong(2)),i=1;i<11;i++)$("#labels"+i).hide(),$("#speaker-"+i).hide()}
...
prepareActivityArrays();isThisH=new Howl({urls:[isThis],autoplay:!1,onend:function(){yesNoLabAndButtons()}});

$("#introBlueGoArra").on("click",function(){clickS.play(),$("#contentSign").empty(),defaultStatusString=modStrings[53],setStatusString(defaultStatusString),html='<div id="sign3_count">'+count+"/"+numPics+'</div><div id="contentSignPicLab"><div class="introPicG"></div><div id="yesNoLabel"><div id="yesNoLabelText"></div></div></div>',$("#contentSign").append(html),html='<div id="sign3_yes"><div id="graphic3_yes"></div></div><div id="sign3_no"><div id="graphic3_no"></div></div>',$("#contentSign").append(html),$("#sign3_yes,#sign3_no").hide(),addStars(1),

$("#sign3_yes, #sign3_no").on("touchstart mouseenter",function(e){e.preventDefault();
$(this).css({"background":"white","border":"2px solid black"});
}).on("mouseleave",function(){
$(this).css({"background":groundCol,"border":"2px solid "+strokeCol});
});
$("#sign3_yes").on("click",function(e){e.preventDefault();$(this).css({"background":groundCol,"border":"2px solid "+strokeCol});onClickYes()});
$("#sign3_no").on("click",function(e){e.preventDefault();$(this).css({"background":groundCol,"border":"2px solid "+strokeCol});onClickNo()});
yesNoPicAndQ(picNum)});

}

function yesNoPicAndQ(i){
console.log('yesNoPicAndQ, count = '+count+', i='+i);
if(count+=1,count>=numPics+1)onYesNoFinished();else{$("#sign3_count").text(count+"/"+numPics);var t='<img class="introPic" src="'+bigPicsURL+picsarray[i]+'.png" width="310px" height="310px" />';$(".introPicG").html(t),isThisH.play()}}
...
function onClickYes(){console.log('onClickYes');correct=1==answerTrue?!0:!1,havingAnswered()}
function onClickNo(){console.log('onClickNo');correct=0==answerTrue?!0:!1,havingAnswered()}
function havingAnswered(){setTimeout(function(){havingAnswered2()},400),1==correct?(highPop.play(),rightAnswers+=1,starRight(1)):(wrong.play(),wrongOverlay("yesNo"),starWrong(1))}
